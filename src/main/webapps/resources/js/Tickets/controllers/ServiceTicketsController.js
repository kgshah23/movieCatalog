...
//		$scope.openticketStatus.id = 5; //$scope.ticketState
...
//		}
       ...
       /*********************************** Get list of tickets ***********************************/
		...
		$scope.serviceticketinterval = $interval(function() {
			console.log('=================================in timer serv tickets fn======================');
			$scope.getListOfTickets();
			$scope.getListOfClosedTickets();
			console.log('serv in');
		}, 10000);
...
//			alert('ddddddddddddddddddsdfdg');
			$interval.cancel($scope.serviceticketinterval);
		});
		...
		$scope.csvOpenTicketHeaders =  ['Ticket ID','Creation Timestamp','Description','Ticket Type','Customer','Site/Location','Expected Time','Ticket State','Restored Timestamp','Assigned To']; // this array contains the headers for csv
		$scope.csvOpenTickets = []; // data for the csv
		
		$scope.csvClosedTicketHeaders =  ['Ticket ID','Closed Timestamp','Description','Ticket Type','Customer','Site/Location','Ticket State','Last Assignee']; // this array contains the headers for csv
		$scope.csvClosedTickets = []; // data for the csv
		
		$scope.updateCsvTickets = function(isOpen){
			try{
				var columns = isOpen ? ['id', 'createdDate', 'description', 'ticketType', 'customerName', 
										'siteandlocation', 'Expected Time', 'state', 'restoredTimestamp', 'Assigned To'] 
									: ['id', 'createdDate', 'description', 'ticketType', 'customerName', 'siteandlocation',
									'state', 'assignedTo'];
				ticketService.registerCtrlr(this);					
				var tickets = ticketService.fetchTicketsForCsv($scope.ticketType,
